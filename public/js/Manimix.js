...
/* splitText (dynamic, supports 'line')
   NOTE: improved line-wrapping to create a block "wrapper" with
   an inline-block inner container so animations on the wrapper
   (translate/opacity/etc) behave consistently with timeline.
*/
function splitText(element, options = { type: 'chars' }) {
  ...
  function wrapLines(spans) {
    // Create a block wrapper for each computed line, but place the
    // actual inline content inside an inline-block inner element.
    // This keeps the line's measured width and allows transforming
    // the wrapper (block) without re-wrapping the inner words.
    const lines = computeLines(spans);
    if (!lines.length) return [];
    const wrappers = [];
    for (const line of lines) {
      const wrapper = document.createElement('span');
      wrapper.setAttribute('data-animix-line', '');
      // block-level wrapper: acts like a line box for transforms
      wrapper.style.display = 'block';
      wrapper.style.width = '100%';
      wrapper.style.overflow = 'visible';
      wrapper.style.pointerEvents = 'auto';

      // inner container preserves the inline width of the original line
      const inner = document.createElement('span');
      inner.setAttribute('data-animix-line-inner', '');
      ...
      try {
        ...
        // move line spans into inner container, then append inner to wrapper
        for (const sp of line) {
          try { inner.appendChild(sp); } catch (e) {}
        }
        try { wrapper.appendChild(inner); } catch (e) {}
  ...
  let computedLines = computeLines(allSpans);
  ...
  function update() {
    ...
    computedLines = computeLines(allSpans);
    charsWords = collectByAttr(allSpans);

    // Prepare lines for return: prefer wrapper elements (blocks) when requested
    let returnLines;
    if (type === 'line' || type === 'lines') {
      // return wrapper elements so users animate the full line block
      returnLines = lineWrappers.length ? lineWrappers.slice() : computedLines.map(l => l.slice());
    } else {
      returnLines = computedLines.map(l => l.slice());
    }

    return { chars: charsWords.chars.slice(), words: charsWords.words.slice(), lines: returnLines };
  }

  // Prepare lines for initial return as well
  let returnLines;
  if (type === 'line' || type === 'lines') {
    returnLines = lineWrappers.length ? lineWrappers.slice() : computedLines.map(l => l.slice());
  } else {
  ...
  return { chars: charsWords.chars.slice(), words: charsWords.words.slice(), lines: returnLines, restore, update, _internalSpans: allSpans };
}
...
/* Add convenience helper methods to Timeline prototype:
   - to, from, fromTo wrappers that behave like animix.to/from/fromTo
   - setAt convenience setter
   These wrappers call animix.* helpers, pause resulting tween(s), and insert them into the timeline at the requested position.
*/
(function addTimelineHelpers(){
  try {
    if (typeof animix !== 'undefined' && typeof animix.timeline === 'function') {
      const proto = animix.timeline().constructor.prototype;

      // to: mirrors animix.to behavior (handles individual/controller returning arrays or GroupController)
      if (proto) {
        proto.to = function(target, durationOrVars, maybeVars, position) {
          // call animix.to exactly as the top-level API would
          const res = animix.to(target, durationOrVars, maybeVars);
          const pos = (typeof position === 'undefined') ? this.duration : this._parsePosition(position);

          // animix.to can return:
          // - a Tween instance
          // - an array of Tweens (when individual requested and returned without controller)
          // - a GroupController (when controller requested)
          if (Array.isArray(res)) {
            for (const t of res) {
              try { if (typeof t.pause === 'function') t.pause(); } catch(e){}
              this.items.push({ tween: t, position: pos });
              this.duration = Math.max(this.duration, pos + (t.duration || 0));
            }
            return this;
          } else if (res && typeof res.seek === 'function') {
            try { if (typeof res.pause === 'function') res.pause(); } catch(e){}
            this.items.push({ tween: res, position: pos });
            this.duration = Math.max(this.duration, pos + (res.duration || 0));
            return this;
          } else if (res && typeof res.getTweens === 'function') {
            const list = res.getTweens();
            for (const t of list) {
              try { if (typeof t.pause === 'function') t.pause(); } catch(e){}
              this.items.push({ tween: t, position: pos });
              this.duration = Math.max(this.duration, pos + (t.duration || 0));
          ...
          // fallback: return this for chaining
          return this;
        };
      }

      // fromTo: wrap animix.fromTo and insert returned tween(s)
      if (proto && !proto.fromTo) {
        proto.fromTo = function(target, durationOrFromVars, maybeFromVars, maybeToVars, position) {
          let duration = durationOrFromVars, fromVars = maybeFromVars, toVars = maybeToVars;
          if (typeof durationOrFromVars === 'object' && typeof maybeFromVars === 'object' && maybeToVars === undefined) {
            fromVars = durationOrFromVars;
            toVars = maybeFromVars;
            duration = undefined;
          }
          fromVars = fromVars || {};
          toVars = toVars || {};
          const res = animix.fromTo(target, duration, fromVars, toVars);
          const pos = (typeof position === 'undefined') ? this.duration : this._parsePosition(position);
      ...
      if (proto && !proto.fromto && proto.fromTo) proto.fromto = proto.fromTo;

      // from: wrap animix.from and insert resulting tween(s)
      if (proto && !proto.from) {
        proto.from = function(target, durationOrFromVars, maybeFromVars, position) {
          let duration = durationOrFromVars, fromVars = maybeFromVars;
          if (typeof durationOrFromVars === 'object' && maybeFromVars === undefined) { fromVars = durationOrFromVars; duration = undefined; }
          fromVars = fromVars || {};
          const res = animix.from(target, duration || undefined, fromVars);
          const pos = (typeof position === 'undefined') ? this.duration : this._parsePosition(position);
      ...
      // setAt: convenience to schedule immediate sets; alias: set
      if (proto && !proto.setAt) {
        proto.setAt = function(target, vars = {}, position) {
          const pos = (typeof position === 'undefined') ? this.duration : this._parsePosition(position);
          const setterTween = {
            duration: 0.000001,
            seek: function(progress) {
              if (progress > 0) {
                try { animix.set(target, vars); } catch(e){}
              }
            },
            pause: function(){},
            play: function(){},
          ...
          this.items.push({ tween: setterTween, position: pos });
          this.duration = Math.max(this.duration, pos + setterTween.duration);
          return this;
        };
      }
      if (proto && !proto.set) proto.set = proto.setAt;
    }
  } catch(e) {
    /* ignore helper registration failures */
  }
})();
